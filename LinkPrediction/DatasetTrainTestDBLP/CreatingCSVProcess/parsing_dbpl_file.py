    ...
    def startElement(self,tag,attributes):
        ...
        if tag == 'article':
            if (articles < 1): 
                ...
                if authorsList:
                    writer.writerow([articles, journ, authorsList])
                self.child = True
                self.parent = tag
                articles = articles + 1
                authorsList = []
...
if __name__=='__main__':
    ...
    journals = set()
    with open('OutputData/co-authorNetwork.csv', 'w', newline='') as file:
        ...
        writer.writerow([articles, journ, authorsList]) #ultima riga da scrivere fuori dal parser
